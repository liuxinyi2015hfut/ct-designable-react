...
var useResizeEffect = function (container, content, engine) {
    ...
    engine.subscribeTo(core_1.DragMoveEvent, function (e) {
        ...
        var factorX = shared_1.calcSpeedFactor(distanceX, 10);
        var factorY = shared_1.calcSpeedFactor(distanceY, 10);
        updateSize(deltaX, deltaY);
        if (distanceX <= 10) {
            if (!animationX) {
                animationX = shared_1.createUniformSpeedAnimation(1000 * factorX, function (delta) {
        ...
        if (distanceY <= 10) {
            if (!animationY) {
                animationY = shared_1.createUniformSpeedAnimation(300 * factorY, function (delta) {
...
exports.ResponsiveSimulator = reactive_react_1.observer(function (props) {
    var container = react_1.useRef();
    var content = react_1.useRef();
    var prefix = hooks_1.usePrefix('responsive-simulator');
    var screen = hooks_1.useScreen();
    hooks_1.useDesigner(function (engine) {
        useResizeEffect(container, content, engine);
    });
    return (react_1.default.createElement("div", __assign({}, props, { className: classnames_1.default(prefix, props.className), style: __assign({ height: '100%', width: '100%', minHeight: 100, position: 'relative' }, props.style) }),
