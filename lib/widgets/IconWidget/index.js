...
require("./styles.less");
var IconContext = react_1.createContext(null);
var isNumSize = function (val) { return /^[\d.]+$/.test(val); };
exports.IconWidget = reactive_react_1.observer(function (props) {
    var _a, _b, _c;
    var theme = hooks_1.useTheme();
    var context = react_1.useContext(IconContext);
    var registry = hooks_1.useRegistry();
    var prefix = hooks_1.usePrefix('icon');
    ...
    var takeIcon = function (infer) {
        ...
        else if (shared_1.isPlainObj(infer)) {
            if (infer[theme]) {
    ...
    var renderTooltips = function (children) {
        if (!shared_1.isStr(props.infer) && (context === null || context === void 0 ? void 0 : context.tooltip))
            return children;
        var tooltip = props.tooltip || registry.getDesignerMessage("icons." + props.infer);
        if (tooltip) {
            var title = react_1.default.isValidElement(tooltip) || shared_1.isStr(tooltip)
                ? tooltip
                : tooltip.title;
            var props_1 = react_1.default.isValidElement(tooltip) || shared_1.isStr(tooltip)
                ? {}
                : shared_1.isObj(tooltip)
                    ? tooltip
                    : {};
    ...
    if (!props.infer)
        return null;
    return renderTooltips(react_1.default.createElement("span", __assign({}, props, { className: classnames_1.default(prefix, props.className), style: __assign(__assign({}, props.style), { cursor: props.onClick ? 'pointer' : (_c = props.style) === null || _c === void 0 ? void 0 : _c.cursor }) }), takeIcon(props.infer)));
});
