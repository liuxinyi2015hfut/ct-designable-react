...
require("./styles.less");
exports.OutlineTreeNode = reactive_react_1.observer(function (_a) {
    var _b, _c;
    var node = _a.node, className = _a.className, style = _a.style, workspaceId = _a.workspaceId;
    var prefix = hooks_1.usePrefix('outline-tree-node');
    var engine = hooks_1.useDesigner();
    var ref = react_1.useRef();
    var ctx = react_1.useContext(context_1.NodeContext);
    var request = react_1.useRef(null);
    var cursor = hooks_1.useCursor();
    var selection = hooks_1.useSelection(workspaceId);
    ...
    react_1.useEffect(function () {
        return engine.subscribeTo(core_1.DragMoveEvent, function () {
            ...
            if (!ref.current)
                return;
            if (closestNodeId === id && closestDirection === core_1.ClosestPosition.Inner) {
    ...
    var renderTitle = function (node) {
        if (shared_1.isFn(ctx.renderTitle))
    ...
    var renderActions = function (node) {
        if (shared_1.isFn(ctx.renderActions))
            return ctx.renderActions(node);
    };
    return (react_1.default.createElement("div", { style: style, ref: ref, className: classnames_1.default(prefix, className, 'expanded'), "data-designer-outline-node-id": node.id },
        react_1.default.createElement("div", { className: prefix + '-header' },
            ...
            react_1.default.createElement("div", { className: prefix + '-header-content' },
                ...
                react_1.default.createElement("div", { className: prefix + '-header-actions', "data-click-stop-propagation": true },
                    renderActions(node),
                    node !== node.root && (react_1.default.createElement(IconWidget_1.IconWidget, { className: classnames_1.default(prefix + '-hidden-icon', {
                            hidden: node.hidden,
